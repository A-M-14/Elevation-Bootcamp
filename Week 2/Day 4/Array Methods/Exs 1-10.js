...
// }
orangeVegetables.forEach(ov => console.log(ov.name)) // NEW WORLD
...
people.forEach(increaseSalaryIfGoodPerformance);

function increaseSalaryIfGoodPerformance(person){
...
// #1
let onlyEmailAndCompanyArray = users.map(u => { return { email: u.email, companyName: u.company.name}});



// #2
let usersZipCodeStartsWith5 = users.filter(u => u.address.zipcode[0] === '5');



// #3
let usersIdsThatZipCodeStartsWith5 = users
...
// #4
let usersNamesStartWithC = users
...
// #5
let isEveryOneInSouthChristy = users.some(u => u.address.city === "South Christy");



// #6
const findUserBySuite = (suiteToFind) => users.find(u => u.address.suite === suiteToFind)
                        .company.name;

console.log(findUserBySuite("Apt. 950"));



// #7
const someData = function(user){
...
let gradeCount = studentScores.reduce((acc, score) => {
  let grade;
  if (score >= 90) grade = 'A';
...
console.log(`Total cost of cart items: ${totalCost}`);